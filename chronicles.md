...
## Current State
**Main app Phases 0+1 built and merged to `master`; the 76-tile collection is authored. Next: write the Phase 2 spec (UI core loop) and implement.** The motivation engine (`app/motivation.js`) is a **pure projection** `project(inputs,{today}) → derivedState`; Phase 1 handles Played + Missed→break, streak/Momentum/points/collection/recovery-dim. `app/theme.js` holds the full world-tour Collection (76 tiles, 0→100k pts ≈ 3 years). `test/` is a `node --test` suite (15/15 green: 13-case matrix + smoke); root `package.json` = `type:module` + test script, zero deps, no build. The browser shell (`index.html`/`main.js`) is deliberately deferred to Phase 2 so the working placeholder keeps running until the real app replaces it (first real deploy = Phase 2).

Design is fully captured in four docs: `main-app-ux.md` (the *what*), `main-app-architecture.md` (the *how*), `main-app-implementation.md` (canonical 6-phase roadmap, Phases 0+1 marked done), `main-app-phase-1.md` (the built spec). Locked decisions still governing the build: pure-projection engine with injected clock+RNG (determinism in engine, randomness in the impure shell); persist only facts, recompute the rest; **storage model** — a "session" is just mic-listening on/off, the live session is in-memory and flushes detected seconds into always-valid `{start,end,playedSec}` records on a throttle (no "running" flag), so today's total survives a reload and raw session data is kept for analysis; "your usual" anchor = median of recent **daily** played totals; Momentum look-ahead `dayMomentum(D)=tier(streakEntering(D)+played)`; Collection costs are cumulative thresholds with capped gap-growth. App will be single-page `index.html` + in-page views with a gated `parent.html`; `detector.js`/`settings.js` stay untouched classic globals read as `window.CelloDetector`.

Detection side unchanged; open questions stand (staccato latency fix, speech rejection at the operating point, Stage 2 in reserve). Deploys are manual, so the merged engine/tests don't touch the live placeholder.

Repo structure: `app/` (`index.html` placeholder, `settings.html` tuning UI, `detector.js`, `settings.js`, **`motivation.js`** engine, **`theme.js`** Collection data; no build step), **`test/`** (node --test) + **`package.json`**, `docs/` (detection pipeline, platform foundations, **main-app-ux / -architecture / -implementation / -phase-1**), `chronicles.md` (history), `README.md` (door).
