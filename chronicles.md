...
## Current State
Stage 1 (pitch stability gate) deployed to `https://cello.mavko.consulting`. First field test surfaced a **staccato false-negative** problem (short notes missed entirely), now addressed by reducing detection latency — see 2026-05-31 "staccato latency" entry. Awaiting re-test on iPhone to confirm staccato is caught without re-admitting speech, and whether the existing `(threshold, tolerance, duration)` operating point still rejects conversational speech. Open lever in reserve: smaller `FFT_SIZE` (2048) for sub-50ms notes, at the cost of low-register resolution. If sustained voiced sounds (humming, TV voiceover) still slip through, proceed to Stage 2 (harmonic extent).
