...
## Current State
**Main app: fully designed (UX + architecture + roadmap + Phase 1 spec), ready to implement.** Four docs drive it: `main-app-ux.md` (the *what*), `main-app-architecture.md` (the *how*), `main-app-implementation.md` (canonical 6-phase roadmap), `main-app-phase-1.md` (build-ready spec for Phase 0 scaffolding + Phase 1 engine — input schema, `project()` contract, Momentum/points/collection math, 13-test matrix). **Immediate next step (user's choice of order): implement Phase 0+1, or first draft the full ~76-tile world-tour collection list (pure data, needed by Phase 2).**

Key architecture locked: the brain (`app/motivation.js`) is a **pure projection** `project(inputs,{today}) → derivedState` — persist only facts, recompute everything else (streak, Momentum, points, collection, freeze, recovery). Clock+RNG injected; randomness in the impure shell, determinism in the engine; makes it unit-testable and "log a lesson un-breaks yesterday" a free replay consequence. Setup: native ES modules + `node --test` (zero deps, minimal root `package.json` = test script only — not a build). Main app = single-page `index.html` + in-page views; gated `parent.html` separate. `detector.js`/`settings.js` stay **untouched classic globals**, read as `window.CelloDetector`.

**Storage model locked (this session):** a "session" is *not* a user concept — just mic listening on/off; the user cares about *total played minutes today*, which must survive reload. So the live session is **in-memory only** (detector/timer/wake-lock), and its detected seconds **flush into the current `sessions[]` record on a throttle**; localStorage holds only always-valid `{start, end, playedSec}` records (no "running" flag; atomic whole-object writes). A reload keeps today's total intact (only the mic stops). The `{start,end,playedSec}` log is also kept deliberately as raw data for later analysis. "Your usual" anchor = median of recent **daily** played totals (not per-session — session length is noise). Momentum look-ahead rule LOCKED: `dayMomentum(D)=tier(streakEntering(D)+played)`.
